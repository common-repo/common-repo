...
/// Directory prefixes that should never be checked for prose patterns.
const EXCLUDED_DIRS: &[&str] = &[
    "context/research", // Research notes generated by AI agents
];
...
mod tests {
    ...
    fn test_should_check_file() {
        ...
        assert!(!should_check_file(Path::new("context/research/foo.md")));
        assert!(!should_check_file(Path::new(
            "context/research/nested/bar.rs"
        )));
        // Non-excluded context paths still checked
        assert!(should_check_file(Path::new("context/design.md")));
    }
