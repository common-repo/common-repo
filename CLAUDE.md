...
<when context="quick_validation">`./script/ci` - Quick validation before committing (fmt, clippy, pre-commit, security audits)</when>
<when context="testing_only">`./script/test` - Run tests only</when>
<when context="full_ci">`./script/cibuild` - Full CI validation (update deps + checks + tests)</when>
</quick_setup>
...
<steps>
<step>Verify clean state: Run `git status`, `git stash list`, check for unpushed commits. Ask user if any pending changes exist.</step>
<step>Create feature branch: Checkout main, pull latest changes on main branch, create branch `<type>/<description>-<session-id>`</step>
<step>Start baseline tests: Run `./script/test` with `run_in_background: true` (skip for docs/context-only changes)</step>
<step>Find current task: Read `context/current-task.json` for active work</step>
<step>Review recent history: Run `git log --oneline -5`</step>
<step>Execute: Find first task where `status=pending` and `blocked_by=null`, complete it, update status</step>
</steps>

After making code changes, use `BashOutput` to check background test results. Start a new background test run after edits to verify changes.
...
<critical>Do not start the new task without confirmation - user typically wants fresh sessions.</critical>
</stash_push>
...
<critical>AskUserQuestion tool does not work on Claude Code iOS. Do not use this tool; instead proceed with reasonable defaults or mention alternatives in your response.</critical>
</platform_limitations>
...
<example name="correct_e2e_usage">
```rust
// CORRECT
...
<code_quality>
<critical>Before every commit, run `./script/ci`.</critical>
...
<rule><critical>NEVER commit/push without explicit user approval</critical></rule>
